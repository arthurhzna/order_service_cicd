pipeline {
  ...
  stages {
    ...
    stage('Checkout Code') {
      steps {
        script {
          def repoUrl = 'https://github.com/arthurhzna/order_service_cicd.git'
    ...
    stage('Commit and Push Changes') {
      steps {
        script {
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@github.com/arthurhzna/order_service_cicd.git
    ...
    stage('Deploy to Remote Server') {
      steps {
        script {
          def targetDir = "/home/arthurhozana123/go/order-service"
          ...
          ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
            if [ -d "${targetDir}/.git" ]; then
                ...
                git clone -b "${TARGET_BRANCH}" git@github.com:arthurhzna/order_service_cicd.git "${targetDir}"
                cd "${targetDir}"
            fi
            ...
            sed -i "s/^CONSUL_HTTP_PATH=.*/CONSUL_HTTP_PATH=order-service/" "${targetDir}/.env"
